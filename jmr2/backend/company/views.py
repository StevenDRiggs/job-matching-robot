...
def benefit_page(req, pk):
    ...
    benefit_json = json.dumps({'pk': pk, 'tag': benefit.tag})
...
def position_page(req, pk):
    position = Position.objects.get(pk=pk)
                           position_json = json.dumps({'pk': pk, 'title': title, 'company': position.company.name, 'description': position.description, 'location': position.get_location(), 'relocation_assistance': position.get_relocation_assistance(), 'position_type': position.get_position_type(), 'pay': position.pay, 'pay_frequency': position.get_pay_frequency(), 'hours': position.get_hours(), 'benefits': [{'pk': b.pk, 'tag': b.tag, 'description': b.description} for b in position.benefits.all()], 'tasks': [{'pk': t.pk, 'tag': t.tag} for t in position.tasks.all()], 'skills': {'required': position.get_required_skills(), 'preferred': position.get_preferred_skills(), 'bonus': position.get_bonus_skills()}, 'traits': {'required': position.get_required_traits(), 'preferred': position.get_preferred_traits(), 'bonus': position.get_bonus_traits()})
